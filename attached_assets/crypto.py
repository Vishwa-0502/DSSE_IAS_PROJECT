...
class DSSE:
    ...
    def search(self, keyword, search_index, search_key):
        ...
        # Hash the keyword for searching
        keyword_hash = self._hmac_hash(search_key, keyword.lower())
        
        # Return positions if found, empty list otherwise
        return search_index.get(keyword_hash, [])
        
    def update(self, old_content, new_content, master_key, search_key, nonce):
        """
        Update encrypted content.
        Returns (new_encrypted_content, new_search_index, new_nonce).
        """
        # Decrypt the old content to understand the structure
        decrypted_content = self.decrypt(old_content, nonce, master_key)
        
        # Convert new_content to bytes if it's a string
        if isinstance(new_content, str):
            new_content = new_content.encode('utf-8')
            
        # Encrypt the new content
        new_nonce, new_encrypted_content = self._encrypt_aes(master_key, new_content)
        
        # Create new searchable index
        new_search_index = self._create_keyword_index(new_content, search_key)
        
        # Convert search index to JSON string
        new_search_index_json = json.dumps(new_search_index)
        
        return new_encrypted_content, new_search_index_json, new_nonce
